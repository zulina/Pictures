...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="name"]', "name");
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="message"]', "message");
...
var accordion = function accordion(triggersSelector) {
  ...
  btns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      btns.forEach(function (btn) {
        if (btn.classList.contains('active-style') && btn != _this) {
...
var burger = function burger(menuSelector, burgerSelector) {
  ...
  burgerElem.addEventListener('click', function () {
    // availWidth - ширина для вывода информации
...
/*!**************************************!*\
  !*** ./src/js/modules/calcScroll.js ***!
  \**************************************/
/*! exports provided: modifyBody, default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var calcScroll = function calcScroll() {
  ...
  div.style.height = '50px'; // добавляем скролл в div

  div.style.overflowY = 'scroll';
  div.style.visibility = 'hidden';
  document.body.appendChild(div); // вычисляем ширину скролла

  var scrollWidth = div.offsetWidth - div.clientWidth;
  div.remove(); // const clientWidth = document.documentElement.clientWidth,
...
var checkTextInputs = function checkTextInputs(selector, param) {
...
var drop = function drop() {
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      if (isImage(e.dataTransfer.files[0])) {
        input.files = e.dataTransfer.files; // обрезаем имя файла

        var name = input.files[0].name.split('.')[0];
        name.length < 6 ? name = input.files[0].name : name = name.substring(0, 5) + "..." + input.files[0].name.split('.')[1]; // выбираем предыдущий соседний элемент
...
var filter = function filter() {
  ...
  menu.addEventListener('click', function (e) {
    ...
    var target = e.target; // установка класса активности
    ...
    if (target && target.tagName == "LI") {
      ...
      typeFilter(wrapper.querySelectorAll(".".concat(target.classList[0])));
    }
  }); // ------------------------ вариант 2 ------------------------
  // items.forEach(btn => {
...
/*!*********************************!*\
  !*** ./src/js/modules/forms.js ***!
  \*********************************/
/*! exports provided: clearInputs, default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var forms = function forms() {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      console.log(item.files[0]); // обрезаем имя файла

      var name = item.files[0].name.split('.')[0];
      name.length < 6 ? name = item.files[0].name : name = name.substring(0, 5) + "..." + item.files[0].name.split('.')[1]; // выбираем предыдущий соседний элемент
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // устанавливаем высоту блока чтобы он был фиксированым

      item.parentNode.style.height = getComputedStyle(item.parentNode).height; // добавляем элемент для вывода статуса

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      item.parentNode.appendChild(statusMessage); // скрываем форму

      item.classList.add('animated', 'fadeOutUp');
      setTimeout(function () {
        item.style.display = 'none';
      }, 400); // добавляем картинку статуса

      var statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMessage.appendChild(statusImg); // добавляем текст статуса

      var textMessage = document.createElement('div');
      textMessage.textContent = message.loading;
      statusMessage.appendChild(textMessage);
      var formData = new FormData(item); // если это форма калькулятора
      ...
      var api; // определяем на какой адрес будем оправлять

      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question; // отправляем данные на сервер
      ...
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData).then(function (res) {
        ...
        setTimeout(function () {
          ...
          statusMessage.remove(); // возвращаем видимость формы
...
var mask = function mask(selector) {
  var matrix = '+8 (___) ___ __ __'; // шаблон

  var setCursorPosition = function setCursorPosition(startPos, endPos, elem) {
    elem.focus();
    ...
    if (elem.setSelectionRange) {
      ...
      var range = elem.createTextRange(); // устанавливаем выделение
  ...
  function getPos(startValue, startPos, endValue) {
    var pos = 0; // считаем количество цифр перед курсором после изменений =
    // количество цифр перед курсором до изменений +
    // (количество цифр всего до изменений - количество цифр всего после изменений)

    var nums = startValue.substring(0, startPos).replace(/\D/g, '').length + (endValue.replace(/\D/g, '').length - startValue.replace(/\D/g, '').length); // для каждого символа в номере телефона
    ...
    try {
      for (var _iterator = endValue[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var letter = _step.value;
        // накапливаем индекс позиции
        pos++; // если это цифра, то отнимаем от нужного количества цифр

        if (/[\d]/.test(letter)) nums--; // если нужное количество цифр закончилось, то возвращаем позицию
  ...
  function createMask(event) {
    ...
    if (event.type === 'blur') {
      ...
      var pos = getPos(startValue, startPos, this.value); // const pos = this.value.length;
  ...
  inputs.forEach(function (input) {
    ...
    input.addEventListener('blur', createMask); // input.addEventListener('keypress', createMask);
...
var modals = function modals() {
  var timeout;
  var btnPressed = false; // привязка окна к определенному тригеру
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        ...
        scroll = Object(_calcScroll__WEBPACK_IMPORTED_MODULE_2__["default"])(); // при нажатии на триггер - открываем модальное окно
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        modal.style.display = "block"; // убираем прокрутку страницы

        Object(_calcScroll__WEBPACK_IMPORTED_MODULE_2__["modifyBody"])("hidden", scroll);
      });
    ...
    close.addEventListener('click', function () {
      ...
      Object(_forms__WEBPACK_IMPORTED_MODULE_1__["clearInputs"])(); // modal.style.display = "none";

      Object(_calcScroll__WEBPACK_IMPORTED_MODULE_2__["modifyBody"])("", "0");
  ...
  function showModalByTime(selector, time) {
    timeout = setTimeout(function () {
      ...
      if (display) {
        ...
        Object(_calcScroll__WEBPACK_IMPORTED_MODULE_2__["modifyBody"])("hidden", Object(_calcScroll__WEBPACK_IMPORTED_MODULE_2__["default"])());
      }
    }, time);
  }
  ...
  bindModal('.button-design', '.popup-design', '.popup-design .popup-close'); // при нажатии на Подробнее об услуге

  bindModal('.button-consultation', '.popup-consultation', '.popup-consultation .popup-close'); // при нажатии на Подарок

  bindModal('.fixed-gift', '.popup-gift', '.popup-gift .popup-close', true); // автоматическое открытие окна через время
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function hideImg(block) {
    var img = block.querySelector('img');
    img.src = img.src.slice(0, -6) + '.png'; // исключаем надпись Хит продаж
...
var scrolling = function scrolling(upSelector) {
  ...
  var links = document.querySelectorAll('[href^="#"]'),
      speed = 0.3; // для всех локальных ссылок

  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      event.preventDefault(); // scrollTop - расстояние которое мы уже пролистали и не видим
      ...
      toBlock = document.querySelector(hash).getBoundingClientRect().top,
          start = null; // console.log(`${widthTop} ${toBlock}`);

      requestAnimationFrame(step); // time - время с момента открытия окна сайта
...
var sliders = function sliders(slides, dir, prev, next) {
  var slideIndex = 0,
      paused = false;
  var items = document.querySelectorAll(slides); // устанавливаем анимацию
  ...
  function plusSlides(n) {
    ...
    slideIndex += n; // если дошли до конца или в начало
  ...
  plusSlides(0); // может и не быть кнопок
